...
2026-07-11에 공식 data.go.kr 결과 API로 2026-06-22 이후 1,358개 출주 결과, 134경주를 새로 수집했다. 이 구간은 v5 및 후속 다중 뷰 후보를 고르는 과정에서 사용되지 않은 미접촉 검증 구간이다.

| 정책 | 경주 | top-1 | top-3 | log-loss |
|---|---:|---:|---:|---:|
| v4 말 이력 기준 모델 | 134 | 31.34% | 66.42% | 1.9135 |
| v5 제한 pairwise | 134 | 29.85% | 66.42% | 1.9137 |

v5는 v4보다 2경주, `-1.493%p` 낮았다. paired bootstrap 95% 구간은 `-3.731~0.000%p`, 개선 확률은 0%였다. 따라서 v5 pairwise는 production에서 비활성화하고 v4 순위를 유지한다.
...
- 제한 pairwise의 재실행 결과는 기존 관찰 구간에서 v4 대비 `+0.549%p`였다. 통계적 신뢰구간이 양수여도 절대 `+5.0%p`에 못 미쳐 기각한다.
- 최근·거리·경마장 모델과 pairwise 결합은 기존 관찰 구간에서 v4 대비 `+0.976%p`였지만 절대 `+5.0%p`에 못 미치고 v5 대비 95% 신뢰구간도 0을 포함해 기각한다.
- 기수 감량·마구·마주 이력을 포함한 다중 뷰 후보도 기존 구간에서 강했지만 새 미접촉 134경주에서 v4를 넘지 못해 승격하지 않았다.
