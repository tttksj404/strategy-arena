...
1. Naver Cloud Platform Server or SSD Server
   ...
   - Use an Ubuntu VM, open 80/443, install Docker Engine and the Docker Compose plugin, then run `deploy/korea-vps/deploy.sh`.
...
- `VPS_HOST` is missing.
- SSH cannot connect.
- Docker Engine or the Docker Compose plugin is not installed.
