...
class LiftCandidate:
    ...
    holdout_n: int
    holdout_exact: float
    holdout_board_exact: float
    holdout_lift_pp: float
    holdout_wins: int
    holdout_losses: int
    sign_test_p: float
    worst_year_lift_pp: float
    min_holdout_year_n: int
...
def frame_from_records(records):
    ...
    for record in records:
        ...
        row = {
            ...
            "meet": str(record.get("meet") or ""),
            "race_no": str(record.get("race_no") or ""),
...
def paired_sign_test_p(wins, losses):
    trials = int(wins) + int(losses)
    if trials <= 0:
        return 1.0
    k = min(int(wins), int(losses))
    log_terms = [
        math.lgamma(trials + 1) - math.lgamma(i + 1) - math.lgamma(trials - i + 1) - trials * math.log(2.0)
        for i in range(k + 1)
    ]
...
def eval_lift_candidate(hits, board_hits, train_base, holdout_base, holdout_year_masks, method, rule, mask):
    if method == "board_min":
        return None
    train_mask = train_base & mask
    train_n = int(train_mask.sum())
    if train_n < MIN_TRAIN_N:
        return None
    train_exact = float(hits[train_mask].mean())
    train_board = float(board_hits[train_mask].mean())
    by_year = {}
    for year in HOLDOUT_YEARS:
        year_mask = holdout_year_masks[year] & mask
        n = int(year_mask.sum())
        if n < PROMOTION_MIN_HOLDOUT_YEAR_N:
            return None
        exact = float(hits[year_mask].mean())
        board_exact = float(board_hits[year_mask].mean())
        lift_pp = (exact - board_exact) * 100.0
        if lift_pp <= 0.0:
            return None
        by_year[year] = (n, exact, board_exact, lift_pp)
    holdout_mask = holdout_base & mask
    holdout_n = int(holdout_mask.sum())
    if holdout_n <= 0:
        return None
    holdout_exact = float(hits[holdout_mask].mean())
    holdout_board = float(board_hits[holdout_mask].mean())
    holdout_lift_pp = (holdout_exact - holdout_board) * 100.0
    wins = int((hits[holdout_mask] & ~board_hits[holdout_mask]).sum())
    losses = int((~hits[holdout_mask] & board_hits[holdout_mask]).sum())
    worst_year_lift_pp = min(item[3] for item in by_year.values())
    min_year_n = min(item[0] for item in by_year.values())
    train_lift_pp = (train_exact - train_board) * 100.0
    sign_test_p = paired_sign_test_p(wins, losses)
    status = "WATCH_DIRECTIONAL_LIFT"
    if (
        train_lift_pp >= 0.0
        and min_year_n >= 50
        and holdout_n >= 300
        and holdout_lift_pp >= 0.3
        and sign_test_p <= 0.05
    ):
        status = "PROMOTE_STAT_STRICT_LIFT"
    elif (
        train_lift_pp >= 0.0
        and min_year_n >= 50
        and holdout_n >= 300
        and worst_year_lift_pp >= 0.5
        and holdout_lift_pp >= 1.0
    ):
        status = "PROMOTE_DIRECTIONAL_LIFT"
    return LiftCandidate(
        ...
        train_board_exact=train_board,
        train_lift_pp=train_lift_pp,
        n_2024=by_year["2024"][0],
        exact_2024=by_year["2024"][1],
        board_2024=by_year["2024"][2],
        lift_2024_pp=by_year["2024"][3],
        n_2025=by_year["2025"][0],
        exact_2025=by_year["2025"][1],
        board_2025=by_year["2025"][2],
        lift_2025_pp=by_year["2025"][3],
        n_2026=by_year["2026"][0],
        exact_2026=by_year["2026"][1],
        board_2026=by_year["2026"][2],
        lift_2026_pp=by_year["2026"][3],
        holdout_n=holdout_n,
        holdout_exact=holdout_exact,
        holdout_board_exact=holdout_board,
        holdout_lift_pp=holdout_lift_pp,
        holdout_wins=wins,
        holdout_losses=losses,
        sign_test_p=sign_test_p,
        worst_year_lift_pp=worst_year_lift_pp,
        min_holdout_year_n=min_year_n,
    )
...
def run(args):
    ...
    board_hits = method_hits["board_min"]
    rows = []
    lift_rows = []
    ...
    lift_rows.sort(
        key=lambda r: (
            r.status == "PROMOTE_STAT_STRICT_LIFT",
            r.status == "PROMOTE_DIRECTIONAL_LIFT",
            r.worst_year_lift_pp,
            r.holdout_lift_pp,
            r.holdout_n,
        ),
        reverse=True,
    ...
    risk_flags = {
        ...
        "directional_lift_not_stat_strict": any(row.status == "PROMOTE_DIRECTIONAL_LIFT" for row in lift_rows)
        and not any(row.status == "PROMOTE_STAT_STRICT_LIFT" for row in lift_rows),
    }
    payload = {
        "records": int(len(df)),
        ...
        "directional_lift_count": len(lift_rows),
        "directional_lift_promote_count": sum(
            1 for row in lift_rows if row.status in {"PROMOTE_DIRECTIONAL_LIFT", "PROMOTE_STAT_STRICT_LIFT"}
        ),
        "stat_strict_lift_count": sum(1 for row in lift_rows if row.status == "PROMOTE_STAT_STRICT_LIFT"),
        ...
        "lift_candidates": [asdict(row) for row in lift_rows[: args.limit]],
    ...
    lines = [
        ...
        (
            "directional_lift_count: "
            f"{payload['directional_lift_count']} "
            f"directional_lift_promote_count: {payload['directional_lift_promote_count']} "
            f"stat_strict_lift_count: {payload['stat_strict_lift_count']}"
        ),
    ...
    for row in lift_rows[:80]:
        lines.append(
            f"| {row.status} | {row.method} | {row.rule} | {row.train_lift_pp:+.3f}pp | "
            f"{row.n_2024}/{row.lift_2024_pp:+.3f}pp | {row.n_2025}/{row.lift_2025_pp:+.3f}pp | "
            f"{row.n_2026}/{row.lift_2026_pp:+.3f}pp | {row.holdout_lift_pp:+.3f}pp | "
            f"{row.holdout_wins}-{row.holdout_losses} | {row.sign_test_p:.4f} |"
        )
    lines.extend([
        "",
    ...
    print(json.dumps(
        {
            ...
            for k in [
                ...
                "directional_lift_count",
                "directional_lift_promote_count",
                "stat_strict_lift_count",
                "risk_flags",
            ]
        },
        ensure_ascii=False,
