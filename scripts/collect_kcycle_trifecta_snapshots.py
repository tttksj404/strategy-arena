...
def collect_once(args):
    ...
    for race_no in parse_races(args.races):
        ...
        axis_signal = engine._market_trifecta_axis_signal(board)
        lift_signal = engine._market_trifecta_lift_signal(board)
        market_timing = engine._kcycle_market_timing_policy(args.date, race_no, fetched_at)
        saved = engine.save_kcycle_trifecta_snapshot(
            ...
            signal=signal,
            source="collector",
            snapshot_phase=engine._snapshot_phase_from_market_timing(market_timing),
        ...
        rows.append({
            ...
            "timing_phase": market_timing.get("phase"),
            "minutes_to_start": market_timing.get("minutes_to_start"),
            "fetched_at": fetched_at,
        })
    return rows
