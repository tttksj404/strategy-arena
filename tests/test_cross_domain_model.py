...
class CrossDomainModelTestCase(unittest.TestCase):
    ...
    def test_predict_get_redirects_to_root(self):
        app_module.app.config["TESTING"] = True
        client = app_module.app.test_client()
    ...
    def test_kcycle_rankingpredict_live_failure_uses_cooldown(self):
        ...
        with patch.dict(os.environ, {
            "KCYCLE_RANKINGPREDICT_ENABLED": "1",
            "KCYCLE_RANKINGPREDICT_LIVE_FETCH_ENABLED": "1",
        }, clear=False), \
    ...
    def test_kcycle_rankingpredict_live_fetch_requires_explicit_opt_in(self):
        engine._KCYCLE_RANKINGPREDICT = {}
        with patch.dict(os.environ, {
            "KCYCLE_RANKINGPREDICT_ENABLED": "1",
            "KCYCLE_RANKINGPREDICT_LIVE_FETCH_ENABLED": "0",
        }, clear=False), \
             patch.object(engine, "_resolve_kcycle_tms") as resolve_tms:
            signal = engine._kcycle_rankingpredict_signal(
                {"stnd_yr": "2026", "ymd": "2026.07.03", "meet": "광명", "race_no": "1"},
            )

        self.assertIsNone(signal)
        resolve_tms.assert_not_called()
        engine._KCYCLE_RANKINGPREDICT = None

    def test_kcycle_official_race_clock_is_strictly_bounded(self):
        engine._KCYCLE_RACE_START_CACHE.clear()
        with patch.dict(os.environ, {
            "KCYCLE_RACE_SCHEDULE_LOOKUP_ATTEMPTS": "1",
            "KCYCLE_RACE_SCHEDULE_LOOKUP_TIMEOUT_SEC": "0.6",
        }, clear=False), \
             patch.object(engine, "_resolve_kcycle_tms", return_value=(2026, 27, 1)), \
             patch.object(engine.urllib.request, "urlopen", side_effect=TimeoutError("blocked")) as urlopen:
            start = engine._kcycle_official_race_start("20260711", "6")
