...
class TrifectaSnapshotAuditTestCase(unittest.TestCase):
    def test_audit_recomputes_signal_and_exact_hit(self):
        ...
        self.assertEqual(result["training_classes"]["post_result_archive_join"], 1)
        self.assertEqual(result["signal_deployability"]["watch_only_failed_small_n"], 1)
    ...
    def test_audit_flags_actual_result_joined_to_live_like_snapshot(self):
        board = make_trifecta_candidate_board()
        key = ("20260628", "광명", "07", engine._trifecta_board_hash(board))
        record = {
            "date": "20260628",
        ...
        self.assertFalse(result["ok"])
        self.assertEqual(result["training_classes"]["live_like_pre_result"], 1)
        self.assertEqual(result["critical_failures"]["actual_joined_live_like_risk"], 1)

    def test_audit_flags_v2_live_like_snapshot_without_timing(self):
        board = make_trifecta_candidate_board()
        key = ("20260628", "광명", "07", engine._trifecta_board_hash(board))
        record = {
            "schema": "kcycle_trifecta_snapshot_v2",
            "date": "20260628",
            "meet": "광명",
            "race_no": "7",
            "stnd_yr": "2026",
        ...
        self.assertFalse(result["ok"])
        self.assertEqual(result["timing_quality"]["live_like_missing_timing"], 1)
        self.assertEqual(result["critical_failures"]["missing_timing_for_live_like_v2"], 1)

    def test_audit_flags_post_start_snapshot_marked_as_pre_result(self):
        board = make_trifecta_candidate_board()
        key = ("20260628", "광명", "07", engine._trifecta_board_hash(board))
        record = {
            "schema": "kcycle_trifecta_snapshot_v2",
            ...
            "source": "live_decision",
            "snapshot_phase": "pre_result_market_snapshot",
            "market_timing": {"phase": "post_start", "minutes_to_start": -3.0},
            "board_count": len(board),
            "board_hash": key[3],
            "signal": engine._live_signal_payload(engine._market_trifecta_signal(board)),
            "board": board,
        ...
        self.assertFalse(result["ok"])
        self.assertEqual(result["timing_quality"]["post_start_snapshots"], 1)
        self.assertEqual(result["critical_failures"]["post_start_marked_pre_result"], 1)
