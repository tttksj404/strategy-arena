...
read_remote_env_value() {
  remote_file="$1"
  key="$2"
  ssh "$SSH_TARGET" "if [ -f '$remote_file' ]; then awk -F= -v wanted='$key' '\$1 == wanted { sub(/^[^=]*=/, \"\"); print; exit }' '$remote_file'; fi" || true
}
...
remote_postgres_password="$(read_remote_env_value "$VPS_PATH/deploy/oracle/.env.oracle" POSTGRES_PASSWORD)"
...
SEARCH_LOOP_INTERVAL_SEC=${SEARCH_LOOP_INTERVAL_SEC:-0}
