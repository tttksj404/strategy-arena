...
PAPER3_ONCE=1 for a single tick; default loops hourly. Auto-stop after 24h.
"""
...
ROOT = HERE.parent
STATE = HERE / "state3.json"
TRADES = HERE / "trades3.jsonl"
LOG = HERE / "paper3.log"
...
G_CAP, B_CAP = 50.0, 150.0
G_LEV, SLOTS, THR, HOLD_H = 5, 5, 70, 24
RT_BPS = 16.0
TV, MAX_LEV, LB, TOPK = 0.25, 5, 40, 5
RUN_HOURS = 24
...
def final_summary() -> None:
    ...
    lines = ["# paper3 극한(부스터tv0.25 $150 + G053 $50) 24h 최종", "",
    ...
    (HERE / "PAPER3_FINAL.md").write_text("\n".join(lines) + "\n")


def main() -> None:
    if os.environ.get("PAPER3_ONCE"):
        tick()
        return
