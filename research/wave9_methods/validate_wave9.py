...
from run_wave9 import IDS, RESULTS_DIR, REPORT_DIR


def _require(condition: bool, message: str) -> None:
    if not condition:
        raise RuntimeError(f"wave9 validation failed: {message}")
...
def main() -> int:
    results = RESULTS_DIR / "wave9_results.json"
    report = REPORT_DIR / "wave9_report.md"
    manifest_path = REPORT_DIR / "wave9_manifest.json"
    ...
    for result in payload["results"]:
        _require(result["candidate_id"] in IDS, "candidate id")
        _require(result["data"]["data_valid"] is True, f"data validity {result['candidate_id']}")
        _require(result["data"]["selection_independent"] is False, f"selection independence {result['candidate_id']}")
        _require(result["execution"]["max_gross"] <= 0.600000001, f"gross cap {result['candidate_id']}")
        _require(set(result["gates"]) == {
            "data_validation", "fixed_universe", "oos_split_present", "selection_independent",
            "capital_contract", "oos_activity", "oos_positive", "oos_sharpe", "positive_blocks",
            "stress_positive", "mc_p05", "ruin_probability", "historical_mdd", "block_mdd_p95", "deflated_sharpe",
        }, f"gate set {result['candidate_id']}")
    ...
    _require(manifest["runner_sha256"] == _hash(base / "run_wave9.py"), "runner hash")
    ...
    print(f"WAVE9_VALIDATION_PASS candidates={len(IDS)} eligible=[] selection_independent=false results={results} report={report} manifest={manifest_path} report_sha256={manifest['report_sha256']} results_sha256={manifest['results_sha256']}")
    return 0
