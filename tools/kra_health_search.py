...
DEFAULT_ARCHIVE: Final = Path("/Users/tttksj/kra/data/health_archive")
DEFAULT_REPORT: Final = ROOT / "runs" / "kra_max_winrate_h8_results.json"
...
def run_search(db_path: Path, archive: Path) -> dict:  # noqa: DICT_OK — JSON experiment report
    ...
    frame, health_columns = add_health_features(rated_frame, load_health(archive))
    candidate_columns = [*rated_columns, *health_columns]
    ...
    coverage = frame["health_treatments_90d"].notna().mean()
    return {
        "method": "strictly_pre_date_veterinary_health_plus_time_safe_ratings",
        "baseline": "kra_dual_phase_v4_history",
        "minimum_absolute_top1_lift_pp": MIN_ABSOLUTE_TOP1_LIFT_PP,
        "health_coverage": float(coverage),
