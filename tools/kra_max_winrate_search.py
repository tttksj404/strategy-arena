...
DEFAULT_REPORT: Final = ROOT / "runs" / "kra_max_winrate_h5_results.json"
FOLDS: Final = (
    ("2024H2", "20240101", "20240701", "20250101"),
    ("2025H1", "20240101", "20250101", "20250701"),
    ("2025H2", "20240101", "20250701", "20260101"),
    ("2026_PRE_FRESH", "20240101", "20260101", "20260622"),
)
FRESH_FROM: Final = "20260622"
WEIGHTS: Final = (0.25, 0.5, 0.75, 1.0)
...
def select_candidate(candidates: tuple[CandidateEvidence, ...]) -> CandidateEvidence:
    return max(
        candidates,
        key=lambda candidate: (
            min(candidate.top1_lifts_pp[:3]),
            float(np.mean(candidate.top1_lifts_pp[:3])),
            min(candidate.top3_lifts_pp[:3]),
            -float(np.mean(candidate.logloss_deltas[:3])),
            -candidate.weight,
        ),
    )
...
def _fold_data(
    ...
    for name, train_from, test_from, test_until in FOLDS:
        ...
        estimator, median = _fit(train, baseline_columns, "win", CONFIGS[0])
        baseline = race_normalize(
            test,
            _predict(estimator, median, test, baseline_columns),
        )
        folds.append(FoldData(name, train, test, baseline))
    return folds
...
def _candidate_evidence(
    ...
    name: str,
    weight: float,
) -> tuple[CandidateEvidence, list[np.ndarray]]:
    top1_lifts = []
    top3_lifts = []
    logloss_deltas = []
    scores = []
    for fold, probability in zip(folds, probabilities):
        score = (1.0 - weight) * fold.baseline_probability + weight * probability
...
def run_search(db_path: Path) -> dict:  # noqa: DICT_OK — JSON research report
    ...
    frame, dynamic_columns = build_pedigree_features(baseline_frame, baseline_columns)
    ...
    for spec in SPECS:
        probabilities = []
        for fold in folds:
            estimator, median = fit_candidate(fold.train, dynamic_columns, spec)
            probabilities.append(
                predict_candidate(estimator, median, fold.test, dynamic_columns, spec.family)
            )
    ...
    candidate_estimator, candidate_median = fit_candidate(
        fresh_train, dynamic_columns, spec
    )
    fresh_raw = predict_candidate(
        candidate_estimator, candidate_median, fresh, dynamic_columns, spec.family
    )
    ...
    return {
        "method": "time_safe_pedigree_offspring_strength_chronological_search",
