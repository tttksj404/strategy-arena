...
DEFAULT_ARCHIVE: Final = Path("/Users/tttksj/kra/data/conditioning_archive")
DEFAULT_REPORT: Final = ROOT / "runs" / "kra_max_winrate_h7_results.json"
...
def run_search(db_path: Path, archive: Path) -> dict:  # noqa: DICT_OK — JSON experiment report
    ...
    rated_frame, rated_columns = build_rating_features(baseline_frame, baseline_columns)
    frame, conditioning_columns = add_conditioning_features(
        rated_frame,
        load_conditioning(archive),
    )
    candidate_columns = [*rated_columns, *conditioning_columns]
    ...
    coverage = frame["condition_minutes_2w"].notna().mean()
    return {
        "method": "official_pre_race_two_week_conditioning_plus_time_safe_ratings",
        "baseline": "kra_dual_phase_v4_history",
        "minimum_absolute_top1_lift_pp": MIN_ABSOLUTE_TOP1_LIFT_PP,
        "conditioning_coverage": float(coverage),
        "selection_folds": [fold.name for fold in folds[:3]],
        "confirmation_fold": folds[3].name,
        "fresh_holdout_is_pristine": False,
        "fresh_holdout_note": "20260622-20260711 was already inspected in H1-H6; promotion requires results after 20260711",
